...
export function Suppliers() {
  ...
  const columns = lang === "vi" ? [
    ...
    { key: "email", label: "Email" }, { key: "tax_code", label: "MST" }, { key: "debt", label: "Công nợ", format: money, readOnly: true },
    { key: "status", label: "Trạng thái", isStatus: true }
  ] : [
    { key: "code", label: "Code" }, { key: "name", label: "Supplier Name" }, { key: "phone", label: "Phone" },
    { key: "email", label: "Email" }, { key: "tax_code", label: "Tax Code" }, { key: "debt", label: "Debt", format: money, readOnly: true },
...
export function Warehouses() {
  ...
  const columns = lang === "vi" ? [
    ...
    { key: "manager", label: "Thủ kho" }, { key: "stock_value", label: "Giá trị tồn kho", format: money, readOnly: true }, { key: "status", label: "Trạng thái", isStatus: true }
  ] : [
    { key: "code", label: "Code" }, { key: "name", label: "Warehouse Name" }, { key: "address", label: "Address" },
    { key: "manager", label: "Manager" }, { key: "stock_value", label: "Stock Value", format: money, readOnly: true }, { key: "status", label: "Status", isStatus: true }
  ];
...
export function StockBalance() {
  ...
  const columns = lang === "vi" ? [
    { key: "product", label: "Sản phẩm", isStatus: false }, { key: "sku", label: "SKU", isStatus: false }, { key: "warehouse", label: "Kho", isStatus: false }, { key: "available", label: "Tồn khả dụng", isStatus: false, format: fmt }, { key: "reserved", label: "Đã đặt", isStatus: false, format: fmt }, { key: "incoming", label: "Nhập tới", isStatus: false, format: fmt }, { key: "outgoing", label: "Xuất ra", isStatus: false, format: fmt }, { key: "avgCost", label: "Giá vốn TB", isStatus: false, format: money }, { key: "value", label: "Giá trị", isStatus: false, format: money }
  ] : [
    { key: "product", label: "Product", isStatus: false }, { key: "sku", label: "SKU", isStatus: false }, { key: "warehouse", label: "Warehouse", isStatus: false }, { key: "available", label: "Available", isStatus: false, format: fmt }, { key: "reserved", label: "Reserved", isStatus: false, format: fmt }, { key: "incoming", label: "Incoming", isStatus: false, format: fmt }, { key: "outgoing", label: "Outgoing", isStatus: false, format: fmt }, { key: "avgCost", label: "Avg Cost", isStatus: false, format: money }, { key: "value", label: "Value", isStatus: false, format: money }
  ];
  ...
  return (
    <div className="flex flex-col h-full">
      ...
      <div className="grid grid-cols-3 gap-3 px-5 py-3 bg-white border-b flex-shrink-0" style={{ borderColor: "var(--border)" }}>
        {[
          { label: isReceivable ? (lang === "vi" ? "Tổng phải thu" : "Total receivable") : (lang === "vi" ? "Tổng phải trả" : "Total payable"), value: money(filtered.reduce((a, b) => a + b.amount, 0)), color: "text-slate-900" },
          { label: isReceivable ? (lang === "vi" ? "Đã thu" : "Collected") : (lang === "vi" ? "Đã trả" : "Paid"), value: money(filtered.reduce((a, b) => a + b.paid, 0)), color: "text-emerald-600" },
          { label: lang === "vi" ? "Còn lại" : "Outstanding", value: money(totalRemaining), color: "text-amber-600" },
        ].map(c => (
          ...
          <tbody>
            {filtered.map((r) => (
              <tr key={r.ref} className="border-b hover:bg-slate-50/60" style={{ borderColor: "var(--border)" }}>
                ...
                <td className="px-4 py-2.5 mono font-semibold text-right">{money(r.amount)}</td>
                <td className="px-4 py-2.5 mono text-right text-emerald-600">{money(r.paid)}</td>
                <td className="px-4 py-2.5 mono text-right font-bold text-amber-600">{money(r.remaining)}</td>
...
function buildReportData(key: string, lang: string, live?: LiveReportContext): ReportData {
  ...
  if (live && (key === "Tồn kho hiện tại" || key === "Stock Balance")) {
    ...
    return {
      ...
      kpis: [
        ...
        { label: lang === "vi" ? "Giá trị tồn kho" : "Inventory Value", value: `${money(totalValue)} ₫` },
      ...
      tableRows: rows.map(row => [row.sku ?? "", row.product_name ?? "", row.warehouse_name ?? "", Number(row.qty ?? 0), Number(row.min_qty ?? 0), money(row.value)]),
    }
  }
  ...
  if (live && (key === "Giá trị tồn kho" || key === "Inventory Value")) {
    ...
    return { ...report, kpis: [{ label: vi ? "Tổng giá trị" : "Total Value", value: `${money(totalValue)} ₫` }, { label: vi ? "Giá bán lẻ dự kiến" : "Potential Retail", value: `${money(retailValue)} ₫` }, { label: vi ? "Biên tiềm năng" : "Potential Margin", value: `${retailValue ? ((retailValue - totalValue) / retailValue * 100).toFixed(1) : "0.0"}%` }, { label: vi ? "Số danh mục" : "Categories", value: fmt(rows.length) }], chartData: rows.slice(0, 12).map(([label, value], index) => ({ label, value: value.value / 1000000000, color: ["#2563eb", "#8b5cf6", "#06b6d4", "#10b981", "#f59e0b", "#ef4444"][index % 6] })), tableHeads: vi ? ["Danh mục", "Số SKU", "SL tồn", "Giá nhập TB", "Giá trị"] : ["Category", "SKUs", "Qty", "Avg Cost", "Total Value"], tableRows: rows.map(([category, value]) => [category, value.sku.size, value.qty, money(value.qty ? value.value / value.qty : 0), money(value.value)]) }
  }
  ...
  if (live && (key === "Doanh thu tổng hợp" || key === "Revenue Summary")) {
    ...
    return { ...report, kpis: [{ label: vi ? "Tổng doanh thu" : "Total Revenue", value: `${money(total)} ₫` }, { label: vi ? "Số hóa đơn" : "Invoices", value: fmt(rows.length) }, { label: vi ? "Giá trị trung bình" : "Average", value: `${money(rows.length ? total / rows.length : 0)} ₫` }, { label: vi ? "Phiếu giao" : "Deliveries", value: fmt(live.deliveries.length) }], chartData: Object.entries(daily).slice(-14).map(([label, value]) => ({ label, value: value / 1000000, color: "#2563eb" })), tableHeads: vi ? ["Hóa đơn", "Khách hàng", "Phiếu giao", "Doanh thu", "Trạng thái"] : ["Invoice", "Customer", "Delivery", "Revenue", "Status"], tableRows: rows.slice(0, 200).map(row => [row.ref ?? "", row.customer_name ?? "", row.delivery_ref ?? "", money(row.total ?? row.amount), row.status ?? ""]) }
  }
  if (live && (key === "Lãi gộp" || key === "Gross Profit")) {
    ...
    return { ...report, kpis: [{ label: vi ? "Doanh thu" : "Revenue", value: `${money(revenue)} ₫` }, { label: vi ? "Giá vốn" : "COGS", value: `${money(cost)} ₫` }, { label: vi ? "Lãi gộp" : "Gross Profit", value: `${money(profit)} ₫` }, { label: vi ? "Biên lợi nhuận" : "Margin", value: `${revenue ? (profit / revenue * 100).toFixed(1) : "0.0"}%` }], chartData: [{ label: vi ? "Doanh thu" : "Revenue", value: revenue / 1000000, value2: profit / 1000000, color: "#2563eb" }], tableHeads: vi ? ["Sản phẩm", "Doanh thu", "Giá vốn", "Lãi gộp"] : ["Product", "Revenue", "COGS", "Gross Profit"], tableRows: lines.slice(0, 200).map(item => [item.product_name ?? "", money(Number(item.qty ?? 0) * Number(item.unit_price ?? 0)), money(Number(item.qty ?? 0) * Number(item.unit_cost ?? 0)), money(Number(item.qty ?? 0) * (Number(item.unit_price ?? 0) - Number(item.unit_cost ?? 0)))]) }
  }
  if (live && (key === "Xếp hạng khách hàng" || key === "Customer Ranking")) {
    ...
    return { ...report, kpis: [{ label: vi ? "Khách hàng có doanh thu" : "Revenue Customers", value: fmt(rows.length) }, { label: vi ? "Tổng doanh thu" : "Total Revenue", value: `${money(total)} ₫` }, { label: vi ? "Top 5 chiếm" : "Top 5 Share", value: `${total ? (rows.slice(0, 5).reduce((sum, row) => sum + row.revenue, 0) / total * 100).toFixed(1) : "0.0"}%` }, { label: vi ? "Tổng công nợ" : "Outstanding", value: `${money(rows.reduce((sum, row) => sum + row.debt, 0))} ₫` }], chartData: rows.slice(0, 10).map((row, index) => ({ label: row.name, value: row.revenue / 1000000, color: ["#2563eb", "#8b5cf6", "#06b6d4", "#10b981", "#f59e0b"][index % 5] })), tableHeads: vi ? ["#", "Khách hàng", "Số hóa đơn", "Doanh thu", "Công nợ"] : ["#", "Customer", "Invoices", "Revenue", "Outstanding"], tableRows: rows.map((row, index) => [index + 1, row.name, row.orders, money(row.revenue), money(row.debt)]) }
  }
  if (live && (key === "Xếp hạng sản phẩm" || key === "Product Ranking")) {
    ...
    return { ...report, kpis: [{ label: vi ? "SKU đã bán" : "SKUs Sold", value: fmt(rows.length) }, { label: vi ? "Số lượng bán" : "Units Sold", value: fmt(rows.reduce((sum, row) => sum + row.qty, 0)) }, { label: vi ? "Sản phẩm bán chạy nhất" : "Best Seller", value: rows[0]?.name ?? "—" }, { label: vi ? "Doanh thu" : "Revenue", value: `${money(rows.reduce((sum, row) => sum + row.revenue, 0))} ₫` }], chartData: rows.slice(0, 10).map((row, index) => ({ label: row.name, value: row.revenue / 1000000, color: ["#2563eb", "#8b5cf6", "#06b6d4", "#10b981", "#f59e0b"][index % 5] })), tableHeads: vi ? ["#", "SKU", "Sản phẩm", "SL bán", "Doanh thu", "Biên LN"] : ["#", "SKU", "Product", "Qty", "Revenue", "Margin"], tableRows: rows.map((row, index) => [index + 1, row.sku, row.name, row.qty, money(row.revenue), `${row.revenue ? ((row.revenue - row.cost) / row.revenue * 100).toFixed(1) : "0.0"}%`]) }
  }
  if (live && (key === "Doanh thu theo nhân viên" || key === "Sales by Employee")) {
    ...
    const rows = Object.values(grouped).sort((a, b) => b.revenue - a.revenue)
    const total = rows.reduce((sum, row) => sum + row.revenue, 0)
    return { ...report, kpis: [{ label: vi ? "Nhân viên bán hàng" : "Sales Staff", value: fmt(rows.length) }, { label: vi ? "Giá trị đơn bán" : "Sales Order Value", value: `${money(total)} ₫` }, { label: vi ? "TB mỗi nhân viên" : "Average per Staff", value: `${money(rows.length ? total / rows.length : 0)} ₫` }, { label: vi ? "Nhân viên dẫn đầu" : "Top Performer", value: rows[0]?.name ?? "—" }], chartData: rows.slice(0, 10).map((row, index) => ({ label: row.name, value: row.revenue / 1000000, color: ["#10b981", "#2563eb", "#8b5cf6", "#06b6d4"][index % 4] })), tableHeads: vi ? ["Nhân viên", "Số đơn", "Giá trị đơn", "% Tổng"] : ["Employee", "Orders", "Order Value", "% Total"], tableRows: rows.map(row => [row.name, row.orders, money(row.revenue), `${total ? (row.revenue / total * 100).toFixed(1) : "0.0"}%`]) }
  }
  ...
  if (live && (key === "Xếp hạng nhà cung cấp" || key === "Supplier Ranking")) {
    ...
    return { ...report, kpis: [{ label: vi ? "Nhà cung cấp" : "Suppliers", value: fmt(rows.length) }, { label: vi ? "Tổng giá trị mua" : "Purchase Value", value: `${money(rows.reduce((sum, row) => sum + row.value, 0))} ₫` }, { label: vi ? "Đã thanh toán" : "Paid", value: `${money(rows.reduce((sum, row) => sum + row.paid, 0))} ₫` }], chartData: rows.slice(0, 10).map((row, index) => ({ label: row.name, value: row.value / 1000000, color: ["#8b5cf6", "#2563eb", "#06b6d4", "#f59e0b", "#10b981"][index % 5] })), tableHeads: vi ? ["#", "Nhà cung cấp", "Số PO", "Giá trị", "Đã thanh toán"] : ["#", "Supplier", "POs", "Value", "Paid"], tableRows: rows.map((row, index) => [index + 1, row.name, row.orders, money(row.value), money(row.paid)]) }
  }
  if (live && (key === "Xu hướng mua hàng" || key === "Purchase Trend")) {
    ...
    return { ...report, kpis: [{ label: vi ? "Số tháng có dữ liệu" : "Months", value: fmt(rows.length) }, { label: vi ? "Trung bình/tháng" : "Monthly Average", value: `${money(rows.length ? total / rows.length : 0)} ₫` }, { label: vi ? "Tổng trong kỳ" : "Period Total", value: `${money(total)} ₫` }], chartData: rows.map(([label, value]) => ({ label, value: value.value / 1000000, color: "#7c3aed" })), tableHeads: vi ? ["Tháng", "Số PO", "Giá trị", "Nhà cung cấp lớn nhất"] : ["Month", "POs", "Value", "Top Supplier"], tableRows: rows.map(([monthKey, value]) => [monthKey, value.count, money(value.value), Object.entries(value.suppliers).sort((a, b) => b[1] - a[1])[0]?.[0] ?? "—"]) }
  }
  if (live && (key === "Tổng hợp nhập kho" || key === "GRN Summary")) {
    ...
    return { ...report, kpis: [{ label: vi ? "Tổng phiếu nhập" : "Total GRNs", value: fmt(rows.length) }, { label: vi ? "Số lượng nhập" : "Units Received", value: fmt(rows.reduce((sum, row) => sum + receiptQty(row), 0)) }, { label: vi ? "Giá trị nhập" : "Value Received", value: `${money(rows.reduce((sum, row) => sum + receiptValue(row), 0))} ₫` }, { label: vi ? "Phiếu chưa hoàn tất" : "Incomplete", value: fmt(rows.filter(row => !["completed"].includes(String(row.status).toLowerCase())).length) }], chartData: Object.entries(warehouses).map(([label, value], index) => ({ label, value, color: ["#2563eb", "#8b5cf6", "#06b6d4"][index % 3] })), tableHeads: vi ? ["Phiếu nhập", "Đơn mua/Báo giá", "Nhà cung cấp", "Kho", "Số lượng", "Trạng thái"] : ["GRN", "PO/Quotation", "Supplier", "Warehouse", "Qty", "Status"], tableRows: rows.map(row => [row.ref ?? "", row.po_ref ?? "", row.supplier_name ?? "", row.warehouse_name ?? "", receiptQty(row), row.status ?? ""]) }
  }
  ...
  if (live && (key === "Tổng hợp chi phí" || key === "Expense Summary")) {
    ...
    return { ...report, kpis: [{ label: vi ? "Tổng chi" : "Total Expenses", value: `${money(total)} ₫` }, { label: vi ? "Số khoản chi" : "Payments", value: fmt(payments.length) }, { label: vi ? "Khoản chi trung bình" : "Average Payment", value: `${money(payments.length ? total / payments.length : 0)} ₫` }], chartData: rows.map(([label, value], index) => ({ label, value: value.amount / 1000000, color: ["#ef4444", "#f59e0b", "#8b5cf6", "#2563eb"][index % 4] })), tableHeads: vi ? ["Nhóm chi", "Số giao dịch", "Số tiền"] : ["Expense Group", "Transactions", "Amount"], tableRows: rows.map(([label, value]) => [label, value.count, money(value.amount)]) }
  }
  if (live && (key === "Tuổi nợ phải thu" || key === "Receivable Aging")) {
    ...
    return { ...report, kpis: [{ label: vi ? "Tổng phải thu" : "Total Receivable", value: `${money(total)} ₫` }, { label: vi ? "Hóa đơn mở" : "Open Invoices", value: fmt(rows.length) }], chartData: Object.entries(buckets).map(([label, value]) => ({ label, value: value / 1000000, color: "#f59e0b" })), tableHeads: vi ? ["Hóa đơn", "Khách hàng", "Đến hạn", "Còn phải thu", "Trạng thái"] : ["Invoice", "Customer", "Due", "Outstanding", "Status"], tableRows: rows.slice(0, 200).map(row => [row.ref ?? "", row.customer_name ?? "", row.due_date ?? row.due ?? row.created_at ?? "", money(row.outstanding_amount ?? row.remaining ?? row.outstanding ?? row.total ?? row.amount), row.status ?? ""]) }
  }
  if (live && (key === "Tuổi nợ phải trả" || key === "Payable Aging")) {
    ...
    const total = Object.values(buckets).reduce((sum, value) => sum + value, 0)
    return { ...report, kpis: [{ label: vi ? "Tổng phải trả" : "Total Payable", value: `${money(total)} ₫` }, { label: vi ? "Đơn mua mở" : "Open POs", value: fmt(rows.length) }], chartData: Object.entries(buckets).map(([label, value]) => ({ label, value: value / 1000000, color: "#ef4444" })), tableHeads: vi ? ["Đơn mua", "Nhà cung cấp", "Đến hạn", "Còn phải trả", "Trạng thái"] : ["PO", "Supplier", "Due", "Outstanding", "Status"], tableRows: rows.slice(0, 200).map(row => [row.ref ?? "", row.supplier_name ?? row.supplier ?? "", row.due_date ?? row.due ?? row.created_at ?? "", money(row.outstanding_amount ?? row.remaining ?? row.outstanding ?? row.total), row.payment_status ?? row.status ?? ""]) }
  }
  if (live) return {
...
export function GoodsReceipt() {
  ...
  const columns = lang === "vi" ? [
    { key: "date", label: "DATE", isStatus: false }, { key: "doc_no", label: "DOC_NO", isStatus: false }, { key: "po_no", label: "PO_NO", isStatus: false }, { key: "supplier", label: "SUPPLIER", isStatus: false }, { key: "warehouse", label: "WAREHOUSE", isStatus: false }, { key: "batch_summary", label: "LÔ", isStatus: false }, { key: "receipt_value", label: "GIÁ TRỊ THỰC NHẬP", isStatus: false, format: money }, { key: "status", label: "STATUS", isStatus: true }
  ] : [
    { key: "date", label: "DATE", isStatus: false }, { key: "doc_no", label: "DOC_NO", isStatus: false }, { key: "po_no", label: "PO_NO", isStatus: false }, { key: "supplier", label: "SUPPLIER", isStatus: false }, { key: "warehouse", label: "WAREHOUSE", isStatus: false }, { key: "batch_summary", label: "BATCH", isStatus: false }, { key: "receipt_value", label: "ACTUAL RECEIPT VALUE", isStatus: false, format: money }, { key: "status", label: "STATUS", isStatus: true }
  ];
  ...
  return (
    <div className="flex flex-col h-full">
      ...
      <div className="grid grid-cols-4 gap-3 px-5 py-3 bg-white border-b flex-shrink-0" style={{ borderColor: "var(--border)" }}>
        {[
          { l: lang === "vi" ? "Tổng doanh thu" : "Total Revenue", v: money(totalRevenue), c: "text-blue-700" },
          { l: lang === "vi" ? "Đã thanh toán" : "Paid", v: money(totalPaid), c: "text-emerald-600" },
          { l: lang === "vi" ? "Còn nợ" : "Outstanding", v: money(totalOutstanding), c: "text-amber-600" },
          { l: lang === "vi" ? "Quá hạn" : "Overdue", v: money(totalOverdue), c: "text-red-600" },
        ].map(c => (
          ...
          <tbody>
            {invoices.map(inv => (
              <tr key={inv.id} className="border-b hover:bg-slate-50/60 cursor-pointer group" style={{ borderColor: "var(--border)" }}>
                ...
                <td className="px-4 py-2.5 mono text-right">{money(inv.amount)}</td>
                <td className="px-4 py-2.5 mono text-right text-slate-500">{money(inv.tax)}</td>
                <td className="px-4 py-2.5 mono text-right font-bold text-slate-900">{money(inv.total)}</td>
...
export function CashBook() {
  ...
  const columns = lang === "vi" ? [
    { key: "date", label: "DATE", isStatus: false }, { key: "doc_no", label: "DOC_NO", isStatus: false }, { key: "type", label: "TYPE", isStatus: false }, { key: "amount", label: "AMOUNT", isStatus: false, format: money }, { key: "balance", label: "BALANCE", isStatus: false, format: money }
  ] : [
    { key: "date", label: "DATE", isStatus: false }, { key: "doc_no", label: "DOC_NO", isStatus: false }, { key: "type", label: "TYPE", isStatus: false }, { key: "amount", label: "AMOUNT", isStatus: false, format: money }, { key: "balance", label: "BALANCE", isStatus: false, format: money }
  ];
