...
const steps = [
  ...
  { icon: FileText, vi: "Lập báo giá", en: "Create quotation", detailVi: "Chọn khách hàng, danh mục, số lượng, giá bán, VAT và chiết khấu.", detailEn: "Select a customer, categories, quantities, prices, VAT, and discounts.", color: "indigo" },
  ...
  { icon: Truck, vi: "Phân bổ và giao", en: "Allocate and deliver", detailVi: "Ghép SKU từ tồn hoặc hàng mới, giữ chỗ rồi xuất kho khi giao.", detailEn: "Mix existing/new SKUs, reserve them, then issue stock at delivery.", color: "amber" },
    ...
    <div className="h-full overflow-x-hidden overflow-y-auto bg-slate-50 p-3 sm:p-5">
      <div className="mx-auto max-w-6xl space-y-3 sm:space-y-5">
        ...
        <div className="grid gap-5 lg:grid-cols-2">
          ...
          <GuideSection icon={<FileText size={17} />} title={vi ? "2. Báo giá và bán hàng" : "2. Quotations and sales"} items={vi ? ["Tạo báo giá theo danh mục; chưa cần chốt SKU hoặc nhà cung cấp.", "Sau khi khách chấp thuận, chọn Convert để ghép nhiều SKU từ tồn hiện có và/hoặc hàng nhập mới. Mọi danh mục phải được phân bổ đúng đủ.", "Convert chỉ nhập hàng mới và giữ chỗ toàn bộ số lượng; chưa xuất kho. Hủy ở trạng thái Chờ giao hàng sẽ giải phóng giữ chỗ nhưng hàng đã nhập vẫn còn.", "Chọn Giao hàng để xuất kho, chốt giá vốn FIFO/Bình quân và tạo hóa đơn. PDF/Excel khách hàng không hiển thị giá vốn."] : ["Create quotations by category without fixing a SKU or supplier.", "After acceptance, Convert may combine multiple existing/new SKUs. Every category must be allocated exactly.", "Conversion receives new stock and reserves all allocations but does not issue inventory. Cancellation releases reservations while received stock remains.", "Deliver to issue inventory, finalize FIFO/average COGS, and create the invoice. Customer exports hide cost data."]} />
          <GuideSection icon={<ShoppingCart size={17} />} title={vi ? "3. Mua hàng và nhập kho" : "3. Purchasing and receiving"} items={vi ? ["Tạo PO với nhà cung cấp, kho và chi tiết sản phẩm.", "Duyệt PO rồi chọn Nhận hàng; nhập số lượng và đơn giá thực tế của lần nhận, có thể khác PO.", "Mỗi lần nhận tạo một lớp giá vốn riêng. Sản phẩm theo lô bắt buộc có số lô; có thể ghi ngày sản xuất và hạn sử dụng.", "Chọn FIFO hoặc Bình quân di động tại Cài đặt công ty; tồn kho, điều chuyển và giá vốn bán hàng sẽ dùng cùng phương pháp."] : ["Create a PO with supplier, warehouse, and product lines.", "Approve it and choose Receive Goods; enter the actual quantity and receipt cost, which may differ from the PO.", "Every receipt creates a separate cost layer. Batch-tracked products require a batch number and may record manufacture and expiry dates.", "Choose FIFO or Moving Average in Company Settings; inventory valuation, transfers, and sales COGS use the same method."]} />
