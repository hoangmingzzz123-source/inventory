...
insert into products (id, org_id, sku, barcode, name, category_id, category, brand, unit, cost, price, qty, status)
values ('88888888-8888-4888-8888-888888888888', '11111111-1111-4111-8111-111111111111', 'TEST-SKU-001', 'TEST-BAR-001', 'Sản phẩm kiểm thử', '33333333-3333-4333-8333-333333333333', 'TEST-CAT', 'TEST-BRAND', 'Piece', 100000, 150000, 0, 'Active')
on conflict (id) do update set name = excluded.name, category_id = excluded.category_id,
  category = excluded.category, cost = excluded.cost, price = excluded.price, qty = 0;
...
where org_id = '11111111-1111-4111-8111-111111111111'
  and (
    outbound_ledger_id in (
      select id from inventory_ledger
      where org_id = '11111111-1111-4111-8111-111111111111'
        and product_id = '88888888-8888-4888-8888-888888888888'
    )
    or cost_layer_id in (
...
where org_id = '11111111-1111-4111-8111-111111111111'
  and quotation_id in (
    ...
    where org_id = '11111111-1111-4111-8111-111111111111'
      and customer_id = '66666666-6666-4666-8666-666666666666'
  );
...
delete from quotation_allocations
where org_id = '11111111-1111-4111-8111-111111111111'
  and quotation_id in (
    select id from quotations
    where org_id = '11111111-1111-4111-8111-111111111111'
...
where org_id = '11111111-1111-4111-8111-111111111111'
  and (ref like 'TEST-%' or quotation_id in (
    select id from quotations
    where org_id = '11111111-1111-4111-8111-111111111111'
      and customer_id = '66666666-6666-4666-8666-666666666666'
  ));
...
where org_id = '11111111-1111-4111-8111-111111111111'
  and (ref like 'TEST-%' or po_ref in (
    select id::text from quotations
    where org_id = '11111111-1111-4111-8111-111111111111'
      and customer_id = '66666666-6666-4666-8666-666666666666'
  ));
